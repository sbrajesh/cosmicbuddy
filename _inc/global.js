...
j("form#status-update-form").live('submit', 
		function() {
			j('input#status-update-post').prop( 'disabled', 'disabled' );
...
j("#status-update-post-cncel").live('click', function(ev) {
	...
	j('div.activity, ul.activity-list').click( function(event) {
			...
			function(response) {
				...
				if ( 'fav' == type ) {
					if ( !j('div.item-list-tabs li#activity-favorites').length )
						j('div.item-list-tabs ul li#activity-mentions').before( '<li id="activity-favorites"><a href="#">' + BP_DTheme.my_favs + ' (<span>0</span>)</a></li>');
	...
	j('span.highlight span').click( function() {
		if ( !j('div.help').length ) {
			j(this).parent().after( '<div id="message" class="info help"><p>' +BP_DTheme.mention_explain + '</p></div>' );
	...
	j("#friend-list li a.remove").live('click', function() {
		...
		function(response)
		{
			j('.ajax-loader').toggle();
			j('#friend-list li#uid-' + friend_id).remove();
			j('#invite-list input#f-' + friend_id).prop('checked', false);
		});
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				button.fadeOut( 100, function() {
					if ( j(this).hasClass('accept') ) {
						j(this).html(BP_DTheme.accepted ).fadeIn(50);
						j(this).addClass('accepted');
					} else {
						j(this).html( BP_DTheme.rejected ).fadeIn(50);
...
function bp_init_activity() {
	...
	if ( null != j.cookie('bp-activity-filter') && j('#activity-filter-select').length )
		j('#activity-filter-select select option[value=' + j.cookie('bp-activity-filter') + ']').prop( 'selected', 'selected' );
...
function bp_init_objects(objects) {
	j(objects).each( function(i) {
		if ( null != j.cookie('bp-' + objects[i] + '-filter') && j('li#' + objects[i] + '-order-select select').length )
			j('li#' + objects[i] + '-order-select select option[value=' + j.cookie('bp-' + objects[i] + '-filter') + ']').prop( 'selected', 'selected' );
...
function bp_filter_request( object, filter, scope, target, search_terms, page, extras ) {
	...
	j('div.item-list-tabs li.selected').addClass('loading');
	j('div.item-list-tabs select option[value=' + filter + ']').prop( 'selected', 'selected' );
...
function bp_activity_request(scope, filter) {
	...
	j('#activity-filter-select select option[value=' + filter + ']').prop( 'selected', 'selected' );
